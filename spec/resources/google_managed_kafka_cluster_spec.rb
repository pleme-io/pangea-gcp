...
# Resource: Manages a google_managed_kafka_cluster resource
  ...
  describe ':google_managed_kafka_cluster' do
    context 'with required attributes only' do
      it 'synthesizes valid Terraform JSON' do
        ...
        synth.google_managed_kafka_cluster('test', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        validate_terraform_structure(result, :resource)
        config = validate_resource_structure(result, 'google_managed_kafka_cluster', 'test')
        validate_required_attributes(config, [:capacity_config, :cluster_id, :gcp_config, :location])
      end

      it 'returns a ResourceReference' do
        synth = create_synthesizer
        synth.extend(described_class)
        ref = synth.google_managed_kafka_cluster('test', required_attrs)

        expect(ref).to be_a(Pangea::Resources::ResourceReference)
        expect(ref.resource_type).to eq(:google_managed_kafka_cluster)
      end
      ...
      it 'provides output references' do
        ...
        expect(ref.state).to eq("${google_managed_kafka_cluster.test.state}")
        expect(ref.terraform_labels).to eq("${google_managed_kafka_cluster.test.terraform_labels}")
        expect(ref.update_time).to eq("${google_managed_kafka_cluster.test.update_time}")
      end
    end
    ...
    context 'with all attributes' do
      ...
      it 'synthesizes with optional attributes' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_managed_kafka_cluster('full', all_attrs)
        result = normalize_synthesis(synth.synthesis)
    ...
    context 'optional attributes' do
      ...
      it 'includes labels when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_managed_kafka_cluster('opt', required_attrs.merge(labels: { 'key1' => 'val1' }))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_managed_kafka_cluster', 'opt')
        expect(config).to have_key('labels')
      end
      ...
      it 'omits rebalance_config when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_managed_kafka_cluster('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_managed_kafka_cluster', 'minimal')
        expect(config).not_to have_key('rebalance_config')
      end
    ...
    context 'attribute types' do
      it 'validates expected attribute types' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_managed_kafka_cluster('typed', required_attrs)
        result = normalize_synthesis(synth.synthesis)
    ...
    context 'multiple instances' do
      it 'synthesizes multiple resources independently' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_managed_kafka_cluster('first', required_attrs)
        synth.google_managed_kafka_cluster('second', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        resources = result.dig('resource', 'google_managed_kafka_cluster')
