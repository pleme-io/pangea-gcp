...
# Resource: Manages a google_compute_project_metadata resource
# Source: terraform-apply

require 'spec_helper'

RSpec.describe Pangea::Resources::GoogleComputeProjectMetadata do
  include Pangea::Testing::SynthesisTestHelpers

  let(:required_attrs) { { metadata: { 'key1' => 'val1' } } }

  describe ':google_compute_project_metadata' do
    context 'with required attributes only' do
      it 'synthesizes valid Terraform JSON' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_project_metadata('test', required_attrs)
      ...
      it 'returns a ResourceReference' do
        ...
        expect(ref.resource_type).to eq(:google_compute_project_metadata)
      end
    ...
    context 'attribute types' do
      it 'validates expected attribute types' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_project_metadata('typed', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        config = validate_resource_structure(result, 'google_compute_project_metadata', 'typed')
    ...
    context 'multiple instances' do
      it 'synthesizes multiple resources independently' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_project_metadata('first', required_attrs)
        synth.google_compute_project_metadata('second', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        resources = result.dig('resource', 'google_compute_project_metadata')
