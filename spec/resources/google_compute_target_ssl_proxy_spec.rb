# frozen_string_literal: true
# Code generated by pangea-forge. DO NOT EDIT.
# Resource: Manages a google_compute_target_ssl_proxy resource
# Source: terraform-apply

require 'spec_helper'

RSpec.describe Pangea::Resources::GoogleComputeTargetSslProxy do
  include Pangea::Testing::SynthesisTestHelpers

  let(:required_attrs) { { backend_service: 'test-value', name: 'test-value' } }

  describe ':google_compute_target_ssl_proxy' do
    context 'with required attributes only' do
      it 'synthesizes valid Terraform JSON' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('test', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        validate_terraform_structure(result, :resource)
        config = validate_resource_structure(result, 'google_compute_target_ssl_proxy', 'test')
        validate_required_attributes(config, [:backend_service, :name])
      end

      it 'returns a ResourceReference' do
        synth = create_synthesizer
        synth.extend(described_class)
        ref = synth.google_compute_target_ssl_proxy('test', required_attrs)

        expect(ref).to be_a(Pangea::Resources::ResourceReference)
        expect(ref.resource_type).to eq(:google_compute_target_ssl_proxy)
      end

      it 'provides output references' do
        synth = create_synthesizer
        synth.extend(described_class)
        ref = synth.google_compute_target_ssl_proxy('test', required_attrs)

        expect(ref.id).to eq("${google_compute_target_ssl_proxy.test.id}")
        expect(ref.creation_timestamp).to eq("${google_compute_target_ssl_proxy.test.creation_timestamp}")
        expect(ref.project).to eq("${google_compute_target_ssl_proxy.test.project}")
        expect(ref.proxy_id).to eq("${google_compute_target_ssl_proxy.test.proxy_id}")
        expect(ref.self_link).to eq("${google_compute_target_ssl_proxy.test.self_link}")
      end
    end

    context 'computed-only attributes' do
      it 'excludes computed-only attributes from the resource block' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('test', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        config = validate_resource_structure(result, 'google_compute_target_ssl_proxy', 'test')
        expect(config).not_to have_key('creation_timestamp')
        expect(config).not_to have_key('project')
        expect(config).not_to have_key('proxy_id')
        expect(config).not_to have_key('self_link')
      end
    end

    context 'with all attributes' do
      let(:all_attrs) { required_attrs.merge({ certificate_map: 'test-value', description: 'test-value', proxy_header: 'test-value', ssl_certificates: ['test-value'], ssl_policy: 'test-value' }) }

      it 'synthesizes with optional attributes' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('full', all_attrs)
        result = normalize_synthesis(synth.synthesis)

        config = validate_resource_structure(result, 'google_compute_target_ssl_proxy', 'full')
        expect(config).to have_key('certificate_map')
        expect(config).to have_key('description')
        expect(config).to have_key('proxy_header')
        expect(config).to have_key('ssl_certificates')
        expect(config).to have_key('ssl_policy')
      end
    end

    context 'optional attributes' do
      it 'includes certificate_map when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('opt', required_attrs.merge(certificate_map: 'test-value'))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_compute_target_ssl_proxy', 'opt')
        expect(config).to have_key('certificate_map')
      end

      it 'omits certificate_map when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_compute_target_ssl_proxy', 'minimal')
        expect(config).not_to have_key('certificate_map')
      end
      it 'includes description when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('opt', required_attrs.merge(description: 'test-value'))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_compute_target_ssl_proxy', 'opt')
        expect(config).to have_key('description')
      end

      it 'omits description when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_compute_target_ssl_proxy', 'minimal')
        expect(config).not_to have_key('description')
      end
      it 'includes proxy_header when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('opt', required_attrs.merge(proxy_header: 'test-value'))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_compute_target_ssl_proxy', 'opt')
        expect(config).to have_key('proxy_header')
      end

      it 'omits proxy_header when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_compute_target_ssl_proxy', 'minimal')
        expect(config).not_to have_key('proxy_header')
      end
      it 'includes ssl_certificates when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('opt', required_attrs.merge(ssl_certificates: ['test-value']))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_compute_target_ssl_proxy', 'opt')
        expect(config).to have_key('ssl_certificates')
      end

      it 'omits ssl_certificates when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_compute_target_ssl_proxy', 'minimal')
        expect(config).not_to have_key('ssl_certificates')
      end
      it 'includes ssl_policy when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('opt', required_attrs.merge(ssl_policy: 'test-value'))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_compute_target_ssl_proxy', 'opt')
        expect(config).to have_key('ssl_policy')
      end

      it 'omits ssl_policy when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_compute_target_ssl_proxy', 'minimal')
        expect(config).not_to have_key('ssl_policy')
      end
    end

    context 'attribute types' do
      it 'validates expected attribute types' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('typed', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        config = validate_resource_structure(result, 'google_compute_target_ssl_proxy', 'typed')
        expect(config['backend_service']).to be_a(String)
        expect(config['name']).to be_a(String)
      end
    end

    context 'multiple instances' do
      it 'synthesizes multiple resources independently' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_compute_target_ssl_proxy('first', required_attrs)
        synth.google_compute_target_ssl_proxy('second', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        resources = result.dig('resource', 'google_compute_target_ssl_proxy')
        expect(resources.keys).to contain_exactly('first', 'second')
      end
    end

    context 'CRUD mapping' do
      it 'documents the API endpoints used' do
        # Create: terraform-apply
        # Read:   terraform-refresh
        # Update: terraform-apply
        # Delete: terraform-destroy
        expect('terraform-apply').not_to be_empty
        expect('terraform-refresh').not_to be_empty
        expect('terraform-destroy').not_to be_empty
      end
    end
  end

  it_behaves_like 'a generated pangea resource',
    resource_type: :google_compute_target_ssl_proxy,
    method: :google_compute_target_ssl_proxy,
    required_attrs: { backend_service: 'test-value', name: 'test-value' },
    expected_outputs: [:id, :creation_timestamp, :project, :proxy_id, :self_link],
    sensitive_fields: [],
    immutable_fields: [],
    boolean_fields: []
end
