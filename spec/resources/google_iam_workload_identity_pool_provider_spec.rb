# frozen_string_literal: true
# Code generated by pangea-forge. DO NOT EDIT.
# Resource: Manages a google_iam_workload_identity_pool_provider resource
# Source: terraform-apply

require 'spec_helper'

RSpec.describe Pangea::Resources::GoogleIamWorkloadIdentityPoolProvider do
  include Pangea::Testing::SynthesisTestHelpers

  let(:required_attrs) { { workload_identity_pool_id: 'test-value', workload_identity_pool_provider_id: 'test-value' } }

  describe ':google_iam_workload_identity_pool_provider' do
    context 'with required attributes only' do
      it 'synthesizes valid Terraform JSON' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('test', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        validate_terraform_structure(result, :resource)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'test')
        validate_required_attributes(config, [:workload_identity_pool_id, :workload_identity_pool_provider_id])
      end

      it 'returns a ResourceReference' do
        synth = create_synthesizer
        synth.extend(described_class)
        ref = synth.google_iam_workload_identity_pool_provider('test', required_attrs)

        expect(ref).to be_a(Pangea::Resources::ResourceReference)
        expect(ref.resource_type).to eq(:google_iam_workload_identity_pool_provider)
      end

      it 'provides output references' do
        synth = create_synthesizer
        synth.extend(described_class)
        ref = synth.google_iam_workload_identity_pool_provider('test', required_attrs)

        expect(ref.id).to eq("${google_iam_workload_identity_pool_provider.test.id}")
        expect(ref.deletion_policy).to eq("${google_iam_workload_identity_pool_provider.test.deletion_policy}")
        expect(ref.name).to eq("${google_iam_workload_identity_pool_provider.test.name}")
        expect(ref.project).to eq("${google_iam_workload_identity_pool_provider.test.project}")
        expect(ref.state).to eq("${google_iam_workload_identity_pool_provider.test.state}")
      end
    end

    context 'computed-only attributes' do
      it 'excludes computed-only attributes from the resource block' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('test', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'test')
        expect(config).not_to have_key('deletion_policy')
        expect(config).not_to have_key('name')
        expect(config).not_to have_key('project')
        expect(config).not_to have_key('state')
      end
    end

    context 'with all attributes' do
      let(:all_attrs) { required_attrs.merge({ attribute_condition: 'test-value', attribute_mapping: { 'key1' => 'val1' }, aws: { 'key1' => 'val1' }, deletion_policy: 'test-value', description: 'test-value', disabled: true, display_name: 'test-value', oidc: { 'key1' => 'val1' }, project: 'test-value', saml: { 'key1' => 'val1' }, x509: { 'key1' => 'val1' } }) }

      it 'synthesizes with optional attributes' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('full', all_attrs)
        result = normalize_synthesis(synth.synthesis)

        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'full')
        expect(config).to have_key('attribute_condition')
        expect(config).to have_key('attribute_mapping')
        expect(config).to have_key('aws')
        expect(config).to have_key('deletion_policy')
        expect(config).to have_key('description')
        expect(config).to have_key('disabled')
        expect(config).to have_key('display_name')
        expect(config).to have_key('oidc')
        expect(config).to have_key('project')
        expect(config).to have_key('saml')
        expect(config).to have_key('x509')
      end
    end

    context 'optional attributes' do
      it 'includes attribute_condition when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('opt', required_attrs.merge(attribute_condition: 'test-value'))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'opt')
        expect(config).to have_key('attribute_condition')
      end

      it 'omits attribute_condition when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'minimal')
        expect(config).not_to have_key('attribute_condition')
      end
      it 'includes attribute_mapping when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('opt', required_attrs.merge(attribute_mapping: { 'key1' => 'val1' }))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'opt')
        expect(config).to have_key('attribute_mapping')
      end

      it 'omits attribute_mapping when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'minimal')
        expect(config).not_to have_key('attribute_mapping')
      end
      it 'includes aws when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('opt', required_attrs.merge(aws: { 'key1' => 'val1' }))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'opt')
        expect(config).to have_key('aws')
      end

      it 'omits aws when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'minimal')
        expect(config).not_to have_key('aws')
      end
      it 'includes deletion_policy when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('opt', required_attrs.merge(deletion_policy: 'test-value'))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'opt')
        expect(config).to have_key('deletion_policy')
      end

      it 'omits deletion_policy when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'minimal')
        expect(config).not_to have_key('deletion_policy')
      end
      it 'includes description when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('opt', required_attrs.merge(description: 'test-value'))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'opt')
        expect(config).to have_key('description')
      end

      it 'omits description when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'minimal')
        expect(config).not_to have_key('description')
      end
      it 'includes disabled when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('opt', required_attrs.merge(disabled: true))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'opt')
        expect(config).to have_key('disabled')
      end

      it 'omits disabled when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'minimal')
        expect(config).not_to have_key('disabled')
      end
      it 'includes display_name when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('opt', required_attrs.merge(display_name: 'test-value'))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'opt')
        expect(config).to have_key('display_name')
      end

      it 'omits display_name when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'minimal')
        expect(config).not_to have_key('display_name')
      end
      it 'includes oidc when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('opt', required_attrs.merge(oidc: { 'key1' => 'val1' }))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'opt')
        expect(config).to have_key('oidc')
      end

      it 'omits oidc when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'minimal')
        expect(config).not_to have_key('oidc')
      end
      it 'includes project when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('opt', required_attrs.merge(project: 'test-value'))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'opt')
        expect(config).to have_key('project')
      end

      it 'omits project when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'minimal')
        expect(config).not_to have_key('project')
      end
      it 'includes saml when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('opt', required_attrs.merge(saml: { 'key1' => 'val1' }))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'opt')
        expect(config).to have_key('saml')
      end

      it 'omits saml when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'minimal')
        expect(config).not_to have_key('saml')
      end
      it 'includes x509 when provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('opt', required_attrs.merge(x509: { 'key1' => 'val1' }))
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'opt')
        expect(config).to have_key('x509')
      end

      it 'omits x509 when not provided' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('minimal', required_attrs)
        result = normalize_synthesis(synth.synthesis)
        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'minimal')
        expect(config).not_to have_key('x509')
      end
    end

    context 'boolean fields' do
      [true, false].each do |val|
        it "accepts disabled=#{val}" do
          synth = create_synthesizer
          synth.extend(described_class)
          attrs = required_attrs.merge(disabled: val)
          synth.google_iam_workload_identity_pool_provider("bool_#{val}", attrs)
          result = normalize_synthesis(synth.synthesis)
          config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', "bool_#{val}")
          expect(config['disabled']).to eq(val)
        end
      end
    end

    context 'attribute types' do
      it 'validates expected attribute types' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('typed', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        config = validate_resource_structure(result, 'google_iam_workload_identity_pool_provider', 'typed')
        expect(config['workload_identity_pool_id']).to be_a(String)
        expect(config['workload_identity_pool_provider_id']).to be_a(String)
      end
    end

    context 'multiple instances' do
      it 'synthesizes multiple resources independently' do
        synth = create_synthesizer
        synth.extend(described_class)
        synth.google_iam_workload_identity_pool_provider('first', required_attrs)
        synth.google_iam_workload_identity_pool_provider('second', required_attrs)
        result = normalize_synthesis(synth.synthesis)

        resources = result.dig('resource', 'google_iam_workload_identity_pool_provider')
        expect(resources.keys).to contain_exactly('first', 'second')
      end
    end

    context 'CRUD mapping' do
      it 'documents the API endpoints used' do
        # Create: terraform-apply
        # Read:   terraform-refresh
        # Update: terraform-apply
        # Delete: terraform-destroy
        expect('terraform-apply').not_to be_empty
        expect('terraform-refresh').not_to be_empty
        expect('terraform-destroy').not_to be_empty
      end
    end
  end

  it_behaves_like 'a generated pangea resource',
    resource_type: :google_iam_workload_identity_pool_provider,
    method: :google_iam_workload_identity_pool_provider,
    required_attrs: { workload_identity_pool_id: 'test-value', workload_identity_pool_provider_id: 'test-value' },
    expected_outputs: [:id, :deletion_policy, :name, :project, :state],
    sensitive_fields: [],
    immutable_fields: [],
    boolean_fields: [:disabled]
end
